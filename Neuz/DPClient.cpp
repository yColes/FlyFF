...
//			case SNAPSHOTTYPE_STOP_COLLECT:		OnStopCollect( objid, ar );	break;
			...
			case SNAPSHOTTYPE_MONSTERHUNTING: OnMonsterHuntingSerialize(ar); break;
#endif
			default:
				{
					ASSERT( 0 );
...
#ifdef _MONSTER_HUNTING
			g_WndMng.m_PlayerMonsterHunt.AddMonsterHuntToPlayer(pAttacker, pMover->GetIndex());
#endif
			if( pMover->IsActiveMover() ) 
			{
				BOOL bBossDie = FALSE;
...
#ifdef _MONSTER_HUNTING
void CDPClient::OnMonsterHuntingSerialize(CAr& ar)
{
	g_WndMng.m_PlayerMonsterHunt.Serialize(ar);
}

void CDPClient::SendClaimMonsterHuntReward(std::uint16_t nHuntIndex)
{
	BEFORESENDSOLE(ar, PACKETTYPE_CLAIMMONSTERHUNT, DPID_UNKNOWN);
	ar << nHuntIndex;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif
