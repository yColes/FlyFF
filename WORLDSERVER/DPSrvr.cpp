...
//	ON_MSG( PACKETTYPE_DO_COLLECT, OnDoCollect );
	...
	ON_MSG(PACKETTYPE_CLAIMMONSTERHUNT, &CDPSrvr::OnReqMonsterHuntReward);
#endif
}
...
#ifdef _MONSTER_HUNTING
void CDPSrvr::OnReqMonsterHuntReward(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	CUser* pUser = g_UserMng.GetUser(dpidCache, dpidUser);
	if (IsValidObj(pUser) == FALSE)
		return;

	std::uint16_t nMonsterHuntIndex = 0;
	ar >> nMonsterHuntIndex;

	pUser->m_PlayerMonsterHunt.ClaimRewardsForHunt(pUser, nMonsterHuntIndex);
}
#endif