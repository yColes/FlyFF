...
BOOL CWndWorld::OnCommand( UINT nID, DWORD dwMessage, CWndBase* pWndBase )
{
	...
	case MMI_RENAME_CANCEL:
		...
		case MMI_MONSTER_HUNTING:
		{
			if (g_WndMng.m_pWndMonsterHunt)
				SAFE_DELETE(g_WndMng.m_pWndMonsterHunt);
			g_WndMng.m_pWndMonsterHunt = new CWndMonsterHunt();
			g_WndMng.m_pWndMonsterHunt->Initialize(NULL);
			break;
		}
#endif
		} // switch
